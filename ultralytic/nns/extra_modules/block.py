...
from timm.models.layers import CondConv2d, trunc_normal_, to_2tuple
#from timm.models import named_apply

from .rational_kat_cu.kat_rational import KAT_Group

__all__ = ['C3k2','DyHeadBlock','FuseBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C2f_Faster', 'C3_Faster', 'C3_ODConv', 'C2f_ODConv', 'Partial_conv3', 'C2f_Faster_EMA', 'C3_Faster_EMA', 'C2f_DBB',
           ...
           ,'PConvScalSeqq','CSP_MutilScaleEdgeInformationEnhance','CSP_PTB','MutilScaleEdgeInfoGenetator','ConvEdgeFusion','CSP_MutilScaleEdgeInformationSelect'
           ,'MutilScaleEdgeInfoGenetator','ConvEdgeFusion','PinwheelConv','C2f_AP','HAFB','C2f_StripCGLU'
           ,'PiEIEStem','C2f_DCMB','C2f_DCMB_KAN','EIEStemdsm_mutiv2','EIEStemdsm_mutiv3'
           ]
...
class EIEStemdsm_muti(nn.Module):
    def __init__(self, inc, hidc, ouc) -> None:
        super().__init__()
        
        self.features=nn.Sequential(
                nn.AdaptiveAvgPool2d(hidc),
                Conv(hidc, hidc, 1),
                Conv(hidc, hidc, 3, g=hidc),
            )

        self.dsm = DualDomainSelectionMechanism(hidc)
        ...
        self.DW = nn.Conv2d(hidc, hidc, kernel_size=3, stride=1, padding=1, groups=hidc) ####ymn
        self.conv_x = nn.Conv2d(hidc, hidc, kernel_size=3, stride=2, padding=1, groups=hidc)#ymn
        self.act_x = nn.GELU()
        self.batch_norm = nn.BatchNorm2d(hidc)       
        self.conv2 = Conv(hidc * 2, hidc, 3, 2)          
        self.conv3 = Conv(hidc, ouc, 1)        
        self.conv_ad = nn.Conv2d(hidc, hidc, kernel_size=1)   ###  self.DRFD = DRFD(hidc * 2, hidc)    
        self.Pin2=PinwheelConv(hidc * 2, hidc,3,2)        
        self.Adaptive = nn.AdaptiveAvgPool2d(hidc)   
        #self.ARConv= ARConv(hidc * 2, hidc)
        self.cut = Cut(in_channels=hidc, out_channels=hidc)
        self.conv_ad1 = nn.Conv2d(hidc, hidc*2, kernel_size=1)
    def forward(self, x):
        x1 = self.conv1(x)        
        x_sb1= self.Adaptive(x1) 
        x_sb12 = F.interpolate(x_sb1, size=x1.shape[2:], mode='bilinear', align_corners=False)
      
        x_sb13 =self.conv_ad(x_sb12) 
        x_sb14 = self.DW(x_sb13)
        x_sb= self.sobel_branch(x_sb14) 
        x_pb1= self.cut(x1)
        x_pb = F.interpolate(x_pb1, size=x1.shape[2:], mode='bilinear', align_corners=False)
    
        x2 = torch.cat([x_sb,x_pb ], dim=1)
        x3 = self.conv2(x2)  ###换成DRFD或者Pinweel   换成DRFD,v3换成ARConv,EiEmutinoAR_DyHGN-HSFPN+DIMB_LS其中ARConv变成普通的卷积
        x30 = self.dsm(x3)
        x4 = self.conv3(x30)
        return x4 
class EIEStemdsm_mutiv2(nn.Module):
    def __init__(self, inc, hidc, ouc) -> None:
        super().__init__()
        ...
        self.act_x = nn.GELU()
        self.batch_norm = nn.BatchNorm2d(hidc)       
        self.conv2 = Conv(hidc * 3, hidc, 3, 2)          
        self.conv3 = Conv(hidc, ouc, 1)        
        self.conv_ad = nn.Conv2d(hidc, hidc, kernel_size=1)   ###  self.DRFD = DRFD(hidc * 2, hidc)    
    ...
    def forward(self, x):
        ...
        x_maxpoo1= self.pool_branch(x1)   
        x_maxpoo = self.batch_norm(x_maxpoo1)        
        x2 = torch.cat([x_sb,x_pb,x_maxpoo ], dim=1)
        x3 = self.conv2(x2)  
        x30 = self.dsm(x3)
        x4 = self.conv3(x30)
        return x4 
class EIEStemdsm_mutiv3(nn.Module):
    def __init__(self, inc, hidc, ouc) -> None:
        super().__init__()
        ...
        self.act_x = nn.GELU()
        self.batch_norm = nn.BatchNorm2d(hidc)       
        self.conv2 = Conv(hidc * 4, hidc, 3, 2)          
        self.conv3 = Conv(hidc, ouc, 1)        
        self.conv_ad = nn.Conv2d(hidc, hidc, kernel_size=1)   ###  self.DRFD = DRFD(hidc * 2, hidc)    
        self.Pin2=PinwheelConv(hidc * 2, hidc,3,2)        
        self.Adaptive = nn.AdaptiveAvgPool2d(hidc)   
    ...
    def forward(self, x):
        ...
        x_maxpoo1= self.pool_branch(x1)   
        x_maxpoo = self.batch_norm(x_maxpoo1)        
        x2 = torch.cat([x_sb,x_pb,x_maxpoo,x1], dim=1)
        x3 = self.conv2(x2)  ###换成DRFD或者Pinweel   换成DRFD,v3换成ARConv,EiEmutinoAR_DyHGN-HSFPN+DIMB_LS其中ARConv变成普通的卷积
        x30 = self.dsm(x3)
        x4 = self.conv3(x30)
        return x4 
...
class C2f_Faster_EMA(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(C2f_Faster_EMA(self.c, self.c) for _ in range(n))
...
class PConvScalSeqq(nn.Module):
    def __init__(self, inc, channel):
        super(PConvScalSeqq, self).__init__()
        if channel != inc[0]:
            self.conv0 = Conv(inc[0], channel,1)
        #self.conv1 =  Conv(inc[1], channel,1)
      #  self.Dysample =
        #output= int(channel/2)
        self.PConv0=  PConv(dim=inc[0],ouc=channel,n_div=4,forward='split_cat')
        self.PConv1=  PConv(dim=inc[1],ouc=channel,n_div=4,forward='split_cat')
        #self.conv2 =  Conv(inc[2], channel,1)
        self.PConv2=  PConv(dim=inc[2],ouc=channel,n_div=4,forward='split_cat')
        ...
        self.dysample3 = DySample(channel, 8, 'lp')
    def forward(self, x):
        p3, p4, p5 = x[0],x[1],x[2]
        #print("p3.size()[2:]",p3.size()[2:])
        if hasattr(self, 'conv0'):
            #p3 = self.conv0(p3)
            p3_2 =  self.PConv0(p3)   
        p3_2 =self.dysample1(p3_2)
        p4_2 = self.PConv1(p4)
        p4_2 = self.dysample2(p4_2)
        
        #p4_2 = F.interpolate(p4_2, size=(160,160), mode='nearest')
        p5_2 = self.PConv2(p5)
        p5_2 = self.dysample3(p5_2)
        #p5_2 = F.interpolate(p5_2, size=(160,160), mode='nearest')
...
class EIEStem(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv3(x)
        return x
class PiEIEStem(nn.Module):
    def __init__(self, inc, hidc, ouc) -> None:
        super().__init__()
        
        self.pinw=PinwheelConv(inc, hidc, 3, 2)
        self.conv1 = Conv(inc, hidc, 3, 2)
        self.sobel_branch = SobelConv(hidc)
        self.pool_branch = nn.Sequential(
            nn.ZeroPad2d((0, 1, 0, 1)),
...
class FuseBlock(nn.Module):
    def __init__(
        ...
        ins = int(inp/2)
        self.fuse1 = Conv(out, out, act=False)
        self.fuse2 = Conv(out, out, act=False)
        self.act = h_sigmoid()
...
class C2f_AP(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(APBottleneck(self.c,self.c) for _ in range(n))
...
class DynamicCIncMixerBlock_KAN(nn.Module):
    ...
    def forward(self, x):
        ...
        N, C, H, W = x.size()
        x1 = x + self.drop_path(self.layer_scale_1.unsqueeze(-1).unsqueeze(-1) * self.mixer(self.norm1(x)))
        
        norm2= self.norm2(x1).flatten(2).permute(0, 2, 1)
        mlp0= self.mlp(norm2)
        mlp = mlp0.permute(0, 2, 1).view([-1, C, H, W]).contiguous()
        a1 = self.drop_path(self.layer_scale_2.unsqueeze(-1).unsqueeze(-1) * mlp)
        x2 = x1 + a1
        return x2
class DynamicCIncMixerBlock_KAt(nn.Module):
    def __init__(self, dim, drop_path=0.0):
        super().__init__()
        self.norm1 = nn.BatchNorm2d(dim)
        self.norm2 = nn.BatchNorm2d(dim)
    ...
    def forward(self, x):
        N, C, H, W = x.size()
        x = x + self.drop_path(self.layer_scale_1.unsqueeze(-1).unsqueeze(-1) * self.mixer(self.norm1(x)))
        x1=self.norm2(x).flatten(2).permute(0, 2, 1)
        x2=self.mlp(x1).permute(0, 2, 1).view([-1, C, H, W]).contiguous()
        x = x + self.drop_path(self.layer_scale_2.unsqueeze(-1).unsqueeze(-1) * x2)
        return x
