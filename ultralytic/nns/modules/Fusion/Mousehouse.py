...
class WeightedInject(nn.Module):
    def __init__(
        ...
        self.act = h_sigmoid()
        '''
        self.local_embedding = Conv(global_inp, oup_2, 1, act=False)
        self.global_embedding = Conv(oup_2, oup_2, 1,act=False)
        self.global_act = Conv(oup_2, oup_2, 1, act=False)
        self.act = h_sigmoid()
...
class ChannelSelection_Medium(nn.Module):
    def __init__(self, in_channel_list, out_channels):
        ...
        #self.downsample = nn.functional.adaptive_avg_pool2d
        self.dysample1 = DySample(in_channels=in_channel_list[0], scale=2, style='lp', groups=2, dyscope=False)
        self.dysample2 = DySample(in_channels=in_channel_list[1], scale=2, style='lp', groups=2, dyscope=False)
    ...
    def forward(self, x):
        ...
        x_ll= F.interpolate(self.dysample1.forward_lp(x[0]), size=(H, W), mode='bilinear', align_corners=False)
          
        x_l = self.cv0(x_ll) 
        
        x_mm= F.interpolate(self.dysample2.forward_lp(x[1]), size=(H, W), mode='bilinear', align_corners=False)
        x_m= self.cv1(x_mm)
...
class ChannelSelection_Bottom(nn.Module):
    def __init__(self, in_channel_list, out_channels):
        ...
        self.cv2 = nn.Sequential(SE_SORT(in_channel_list[2], out_channels_2+2) if in_channel_list[
                                                                                    2] > out_channels_2 else nn.Identity(),
                                 Conv(out_channels_2+2, out_channels_2+2, act=nn.ReLU()) if in_channel_list[
                                                                                            2] > out_channels_2
                                 else Conv(in_channel_list[2], out_channels_2+2, act=nn.ReLU())
                                 )
        #self.downsample = nn.functional.adaptive_avg_pool2d
        self.dysample1 = DySample(in_channels=in_channel_list[0], scale=2, style='lp', groups=2, dyscope=False)
        self.dysample2 = DySample(in_channels=in_channel_list[1], scale=2, style='lp', groups=2, dyscope=False)
        self.dysample3 = DySample(in_channels=in_channel_list[2], scale=2, style='lp', groups=2, dyscope=False)
    ...
    def forward(self, x):
        ...
        x_ss= F.interpolate(self.dysample3.forward_lp(x[2]), size=(H, W), mode='bilinear', align_corners=False)
        x_s= self.cv2(x_ss)
