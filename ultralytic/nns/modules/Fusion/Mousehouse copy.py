...
class WeightedInject(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.global_inp = global_inp##64
        inp_2=int(inp/2) ##128
        oup_2=int(oup/2) ##64
        self.local_embedding = Conv(inp, oup_2, 1, act=False)  ###(64,64)
        self.global_embedding = Conv(global_inp, oup_2, 1,act=False) ###(64,64)
        self.global_act = Conv(global_inp, oup_2, 1, act=False)  ##(64,64)
        #self.global_act = Conv(global_inp, oup, 1, act=False)  ##(64,64)
        self.act = h_sigmoid()
    ...
    def forward(self, x):
        ...
        '''
        x_l, x_g = x
        print("x_l.shape,x_g.shape",x_l.shape,x_g.shape)
        gloabl_info = x_g
...
class ChannelSelection_Top(nn.Module):
    def __init__(self, in_channel_list, out_channels):
       ...
       # print('self.downsample=nn.functional.adaptive_avg_pool2d',self.downsample)
        inss=in_channel_list[0]
        self.dysample = DySample(in_channels=in_channel_list[0], scale=2, style='lp', groups=2, dyscope=False)
        #self.final_conv = Conv(out_channels_2 * 3, out_channels, act=nn.ReLU()) 
    def forward(self, x):
        N, C, H, W = x[1].shape
        output_size = (H, W)
        print('x[0]:',x[0].shape)
        
        ss= F.interpolate(self.dysample.forward_lp(x[0]), size=(H, W), mode='bilinear', align_corners=False)
          
        x_l = self.cv0(ss) 
        #x_l = self.cv0(self.downsample(x[0], output_size))  ######换成Dysample
...
class ChannelSelection_Medium(nn.Module):
    ...
    def forward(self, x):
        ...
        #
        s1=self.downsample(x[0], output_size)
        s2=self.downsample(x[1], output_size)
        s3=self.downsample(x[3], output_size)
        
        x_ll= F.interpolate(self.dysample.forward_lp(x[0]), size=(H, W), mode='bilinear', align_corners=False)
          
        x_l = self.cv0(x_ll) 
        
        x_mm= F.interpolate(self.dysample.forward_lp(x[1]), size=(H, W), mode='bilinear', align_corners=False)
        x_m= self.cv1(x_mm)
        #x_l = self.cv0(self.downsample(x[0], output_size))
        #x_m = self.cv1(self.downsample(x[1], output_size))
        #x_n=F.interpolate(self.dysample.forward_lp(x[3]), size=(H, W), mode='bilinear', align_corners=False)
        x_n = self.cv3(F.interpolate(x[3], size=(H, W), mode='bilinear', align_corners=False))
        #x_n= self.cv3(x_n)
...
class ChannelSelection_Bottom(nn.Module):
    ...
    def forward(self, x):
        ...
        #s2=self.downsample(x[1], output_size)
        #s3=self.downsample(x[3], output_size)
        x_l=F.interpolate(self.dysample.forward_lp(x_l), size=(H, W), mode='bilinear', align_corners=False)
        x_l= self.cv0(x_l)
        #x_l = self.cv0(self.downsample(x_l, output_size))
        x_m= F.interpolate(self.dysample.forward_lp(x_m), size=(H, W), mode='bilinear', align_corners=False)
        x_m= self.cv1(x_m)
        #x_m = self.cv1(self.downsample(x_m, output_size))
        x_s= F.interpolate(self.dysample.forward_lp(x_s), size=(H, W), mode='bilinear', align_corners=False)
        x_s= self.cv2(x_s)
