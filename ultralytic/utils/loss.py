...
from .metrics import bbox_iou, probiou,bbox_OIoU,bbox_inner_iou
from .tal import bbox2dist
import math
from ultralytics.utils.dyanmic_atss_assigner import DynamicATSS
class classLossPMFM(nn.Module):
    def __init__(self,loss_fcn, weightsss ):
        super(classLossPMFM,self).__init__()
        self.loss_fcn = loss_fcn
        self.weights = weightsss
        self.reduction = loss_fcn.reduction
        self.loss_fcn.reduction = 'none'  # required to apply SL to each elemen
        
    def forward(self, pred, true, auto_iou=0.5):
        loss = self.loss_fcn(pred, true) ######基础损失
        weights = self.weights[true.long()]  # 根据目标类别应用权重
        
        weighted_loss = loss * weights
        return weighted_loss


class slideAdaptiveThresholdV0(nn.Module):
    def __init__(self, loss_fcn):
        super(slideAdaptiveThresholdV0,self).__init__()
        self.loss_fcn = loss_fcn
        #self.reduction = loss_fcn.reduction
        #self.loss_fcn.reduction = 'none'  # required to apply SL to each elemen
        self.reduction = loss_fcn.reduction
    ...
    def forward(self, pred, true, auto_iou=0.5):
        ...
        mean_true=true.mean()
        std_true=true.std()
        auto_iou=mean_true+0.5*std_true
        auto_iou=torch.clamp(auto_iou,min=0.5,max=1.0)
        if auto_iou :
###############################################################
...
class SlideLoss(nn.Module):
    def __init__(self, loss_fcn):
        ...
        self.loss_fcn.reduction = 'none'  # required to apply SL to each elemen
    def forward(self, pred, true, auto_iou=0.5):
        loss = self.loss_fcn(pred, true)  ###利用IoU计算
        if auto_iou < 0.2:
            auto_iou = 0.2
        b1 = true <= auto_iou - 0.1
        a1 = 1.0
...
class AdaptiveThresholdFocalLoss(nn.Module):
    ...
    ###根据预测概率自适应调整聚焦参数gamma，有助于调制每个样本的损失贡献，使得模型更加关注难以分类的样本
    def __init__(self, loss_fcn, gamma=1.5, alpha=0.25):
        super(AdaptiveThresholdFocalLoss, self).__init__()
        self.loss_fcn = loss_fcn  # must be nn.BCEWithLogitsLoss()
        self.gamma = gamma####控制难易样本
        self.alpha = alpha  ###控制类别不平衡
        self.reduction = loss_fcn.reduction
...
class FocalLoss(nn.Module):
    ...
    def forward(pred, label, gamma=1.5, alpha=0.25):
        ...
        pred_prob = pred.sigmoid()  # prob from logits  预测得分
        p_t = label * pred_prob + (1 - label) * (1 - pred_prob)  #######预测概率
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask,epoch):
        ...
        iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False,epoch=epoch,CIoU=True)#########ymn
        #iou = bbox_OIoU(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, OIoU=True)
        #iou = bbox_inner_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, OIoU=True)
        # UIoU设置为True则代码使用UIoU, Focal同时设置为True 则设置的是FocalUIoU /data/ymn/quesu/ultralytics_sunshi/runs
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk=10):  # model must be de-paralleled
        ...
        self.class_weight=[1.0,2.0,1.0,2.0,1.0]
        m = model.model[-1]  # Detect() module
        #self.base_loss= nn.BCEWithLogitsLoss(reduction='none')
        #self.weighted_loss=classLossPMFM(self.base_loss,self.class_weight)
       # self.bce = slideAdaptiveThresholdV0(self.base_loss)
        
        #self.bce = SlideLoss(nn.BCEWithLogitsLoss(reduction='none'))  # Slide Loss BCEWithLogitsLoss二元交叉熵损失函数
