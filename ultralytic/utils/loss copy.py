...
class classLossPMFM(nn.Module):
    def __init__(self,base_loss,weights ):
        super(classLossPMFM).__init__()
        self.base_loss = base_loss
        self.weights = weights
    def forward(self, pred, true, auto_iou=0.5):
        loss = self.base_loss(preds, targets) ######基础损失
        weights = self.weights[targets.long()]  # 根据目标类别应用权重
...
class slideAdaptiveThresholdV0(nn.Module):
    def __init__(self, loss_fcn):
        super(slideAdaptiveThresholdV0).__init__()
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk=10):  # model must be de-paralleled
        ...
        #self.bce = AdaptiveThresholdFocalLoss(weighted_loss)
