...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        if not isinstance(imgsz, list):
            ...
            imgsz = [imgsz, imgsz]
        try:
            if hasattr(model_cpu, "stride") and model_cpu.stride is not None:
                stride = max(int(model_cpu.stride.max()), 32)
                #stride = max(int(model.stride.max()), 32) if hasattr(model, "stride") else 32
            # 确保最小尺寸为128
            min_size = 128
            calc_size = max(stride, min_size)
            input_tensor = torch.zeros((1, 3, calc_size, calc_size))
            # 使用固定3通道构造输入
             # 计算FLOPs - 关键修改：确保模型和输入都在CPU
            flops, _ = thop.profile(
                model_cpu, 
                inputs=(input_tensor,), 
                verbose=False
            )
            ...
            # 计算最终FLOPs
            flops = flops / 1e9 * 2  # 转换为GFLOPs
            result = flops * imgsz[0] / calc_size * imgsz[1] / calc_size
            return result
            ...
            # 方法2：使用实际尺寸作为备选
            try:
                input_tensor = torch.zeros((1, 3, imgsz[0], imgsz[1]))
                flops, _ = thop.profile(
                    model_cpu, 
                    inputs=(input_tensor,), 
                    verbose=False
