...
def get_inner_iou(box1, box2, xywh=True, eps=1e-7, ratio=0.7):
    ...
    return inter / union
def bbox_inner_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, OIoU=False,CIoU=False, EIoU=False, SIoU=False, ShapeIoU=False, PIoU=False, PIoU2=False, eps=1e-7, ratio=0.7, scale=0.0, Lambda=1.3):
    """
        ...
        if CIoU or DIoU or EIoU or SIoU or PIoU or PIoU2 or ShapeIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                return innner_iou - (rho2 / c2 + v * alpha)  # CIoU
                    ...
                    distance = ((inter_center_x - real_center_x) ** 2 + (inter_center_y - real_center_y) ** 2).clamp_(0)   
                    oiou = innner_iou - (distance / inter+ v* alpha)
                    #uiou= iou - (distance/c2 + v* alpha)
                    return oiou 
            elif EIoU:
...
def xyxy2xywh(x):
    ...
    y[..., 3] = x[..., 3] - x[..., 1]  # height
    return y
def bbox_UIoU_iou(box1,box2, xywh=True, UIoU=True,epoch=1, ratio=0.7,eps=1e-7):
    ...
    if UIoU:
        ...
        # Union Area
            union = w1 * h1 + w2 * h2 - inter + eps
            UIoU_iou = inter/(union+eps) 
            gamma=0.5
            UIoU_iou=torch.pow(inter / (union + eps), gamma)
            return UIoU_iou
def bbox_OIoU(box1, box2, xywh=False, OIoU=False, DIoU=False, CIoU=False, alpha=1,d=0.00,u=0.95,ratio=0.7,eps=1e-7,scale=0.0):
    if xywh:  # transform from xywh to xyxy
            (x1, y1, w1, h1), (x2, y2, w2, h2) = box1.chunk(4, -1), box2.chunk(4, -1)
            w1_, h1_, w2_, h2_ = w1 / 2, h1 / 2, w2 / 2, h2 / 2
            b1_x1, b1_x2, b1_y1, b1_y2 = x1 - w1_, x1 + w1_, y1 - h1_, y1 + h1_
    ...
    inter_y1= b1_y1.minimum(b2_y1)
    inter_y2= b1_y2.maximum(b2_y2)
    
    inter_center_x= (inter_x1 + inter_x2)/2
    inter_center_y= (inter_y1 + inter_y2)/2
    real_center_x = (b2_x1 + b2_x2) / 2
    real_center_y = (b2_y1 + b2_y2) / 2
    
    union = w1 * h1 + w2 * h2 - inter + eps
    #iou = torch.pow(inter/(union+eps),alpha)
    iou = inter / union
    
    if CIoU or DIoU or OIoU:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
    ...
    return iou  # IoU 
def bbox_iou(box1, box2,epoch=1, xywh=True, GIoU=False, DIoU=False, CIoU=False,UIoU=False,Inner=False,Focal=False,Focaler=False,PIoU=False, PIoU2=False,ShapeIoU=False,alpha=1,d=0.00,u=0.95,gamma=0.5,ratio=0.7,eps=1e-7,scale=0.0):
    #bbox_UIoU()
    if xywh:  # transform from xywh to xyxy
        (x1, y1, w1, h1), (x2, y2, w2, h2) = box1.chunk(4, -1), box2.chunk(4, -1)
        w1_, h1_, w2_, h2_ = w1 / 2, h1 / 2, w2 / 2, h2 / 2
        ...
        b2_x1, b2_x2, b2_y1, b2_y2 = x2 - w2_, x2 + w2_, y2 - h2_, y2 + h2_
            ...
            w2, h2 = b2_x2 - b2_x1, b2_y2 - b2_y1 + eps
    epoch=epoch
    UIoU = bbox_UIoU_iou(box1,box2,xywh=False,UIoU=UIoU, epoch=epoch, ratio=0.7)
    inter = (b1_x2.minimum(b2_x2) - b1_x1.maximum(b2_x1)).clamp_(0) * (
    ...
    if CIoU or DIoU or GIoU or PIoU or ShapeIoU:
        ...
        if CIoU or DIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                omiga_h = ww * torch.abs(h1 - h2) / torch.max(h1, h2)
                shape_cost = torch.pow(1 - torch.exp(-1 * omiga_w), 4) + torch.pow(1 - torch.exp(-1 * omiga_h), 4)
                #print('ShapeIoU:',ShapeIoU)
                if  Focal:
                    #print('Focal_ShapeIoU',Focal)
                    return UIoU - distance - 0.5 * shape_cost, torch.pow(inter / (union + eps), gamma) 
                else:    
                    return UIoU - distance - 0.5 * shape_cost
